...
use tox::toxcore::onion::onion_client::*;
    ...
    let network_reader = stream.then(future::ok).filter(|event|
        ...
        match packet {
            Packet::OnionAnnounceResponse(packet) => {
                Box::new(onion_client_c.handle_announce_response(&packet, addr))
                    as Box<dyn Future<Item = _, Error = _> + Send>
            },
            Packet::OnionDataResponse(packet) => {
                Box::new(onion_client_c.handle_data_response(&packet))
            },
            _ => Box::new(future::ok(())),
        }.or_else(|err| {
