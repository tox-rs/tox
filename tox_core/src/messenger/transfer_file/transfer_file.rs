...
impl FileSending {
    ...
    fn recv_from(&self, friend_pk: PublicKey, packet: FileTransferPacket) -> impl Future<Item=(), Error=RecvPacketError> + Send {
        let tx = self.recv_file_control_tx.clone();
        send_to(&tx, (friend_pk, packet))
            .map_err(RecvPacketError::from)
    }

    fn recv_from_data(&self, friend_pk: PublicKey, packet: FileTransferPacket, position: u64) -> impl Future<Item=(), Error=RecvPacketError> + Send {
    ...
    pub fn handle_file_send_request(&self, friend_pk: PublicKey, packet: FileSendRequest)
        ...
        Either::B(self.clone().recv_from(friend_pk, FileTransferPacket::FileSendRequest(packet)))
    }
