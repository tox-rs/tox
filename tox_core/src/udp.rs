...
impl Server {
    ...
    pub async fn handle_packet(&self, packet: Packet, addr: SocketAddr) -> Result<(), HandlePacketError> {
        match packet {
            Packet::PingRequest(packet) => self.dht.handle_ping_req(packet, addr).await,
            Packet::PingResponse(packet) => self.dht.handle_ping_resp(packet, addr).await,
            Packet::NodesRequest(packet) => self.dht.handle_nodes_req(packet, addr).await,
            Packet::NodesResponse(packet) => self.dht.handle_nodes_resp(packet, addr).await,
            Packet::CookieRequest(packet) => self.handle_cookie_request(&packet, addr).await,
            Packet::CookieResponse(packet) => self.handle_cookie_response(&packet, addr).await,
            Packet::CryptoHandshake(packet) => self.handle_crypto_handshake(&packet, addr).await,
            Packet::DhtRequest(packet) => self.dht.handle_dht_req(packet, addr).await,
            Packet::LanDiscovery(packet) => self.dht.handle_lan_discovery(&packet, addr).await,
            Packet::OnionRequest0(packet) => self.dht.handle_onion_request_0(packet, addr).await,
            Packet::OnionRequest1(packet) => self.dht.handle_onion_request_1(packet, addr).await,
            Packet::OnionRequest2(packet) => self.dht.handle_onion_request_2(packet, addr).await,
            Packet::OnionAnnounceRequest(packet) => self.dht.handle_onion_announce_request(packet, addr).await,
            Packet::OnionDataRequest(packet) => self.dht.handle_onion_data_request(packet).await,
            Packet::OnionResponse3(packet) => self.dht.handle_onion_response_3(packet).await,
            Packet::OnionResponse2(packet) => self.dht.handle_onion_response_2(packet).await,
            Packet::OnionResponse1(packet) => self.dht.handle_onion_response_1(packet).await,
            Packet::BootstrapInfo(packet) => self.dht.handle_bootstrap_info(&packet, addr).await,
            Packet::CryptoData(packet) => self.handle_crypto_data(&packet, addr).await,
            Packet::OnionDataResponse(packet) => self.handle_onion_data_response(&packet).await,
            Packet::OnionAnnounceResponse(packet) => self.handle_onion_announce_response(&packet, addr).await,
        }
    }
    ...
    async fn handle_onion_announce_response(
        &self,
        ...
        if let Some(ref onion_client) = self.onion_client {
            onion_client
                .handle_announce_response(packet, IsGlobal::is_global(&addr.ip()))
                .await
    ...
    /// module.
    pub async fn handle_cookie_request(
        &self,
        ...
        if let Some(ref net_crypto) = self.net_crypto {
            net_crypto
                .handle_udp_cookie_request(packet, addr)
                .await
    ...
    pub async fn handle_cookie_response(
        ...
        if let Some(ref net_crypto) = self.net_crypto {
            net_crypto
                .handle_udp_cookie_response(packet, addr)
                .await
                .map_err(HandlePacketError::HandleNetCrypto)
        } else {
            Err(HandlePacketError::NetCrypto)
    ...
    pub async fn handle_crypto_handshake(
        ...
        if let Some(ref net_crypto) = self.net_crypto {
            net_crypto
                .handle_udp_crypto_handshake(packet, addr)
                .await
                .map_err(HandlePacketError::HandleNetCrypto)
        } else {
            Err(HandlePacketError::NetCrypto)
    ...
    pub async fn handle_crypto_data(&self, packet: &CryptoData, addr: SocketAddr) -> Result<(), HandlePacketError> {
        if let Some(ref net_crypto) = self.net_crypto {
            net_crypto
                .handle_udp_crypto_data(packet, addr)
                .await
                .map_err(HandlePacketError::HandleNetCrypto)
        } else {
            Err(HandlePacketError::NetCrypto)
