...
pub const DHT_STATE_BUFFER_SIZE: usize =
    ...
    (
        ...
        (
            // PK size
            32 +
            // SocketAddr maximum size
            19
        ) * KBUCKET_DEFAULT_SIZE as usize
    ) * KBUCKET_MAX_ENTRIES as usize;
...
impl DaemonState {
    ...
    pub async fn serialize_old(server: &Server) -> Vec<u8> {
        ...
        let (_, buf_len) = DhtState(nodes)
            .to_bytes((&mut buf, 0))
            .expect("DhtState(nodes).to_bytes has failed");
    ...
    pub async fn deserialize_old(server: &Server, serialized_data: &[u8]) -> Result<(), DeserializeError> {
        let nodes = match DhtState::from_bytes(serialized_data) {
            Err(error) => return Err(DeserializeError::deserialize(error, serialized_data.to_vec())),
            Ok((_, DhtState(nodes))) => nodes,
        };

        let nodes_sender = nodes.into_iter().map(|node| server.ping_node(node));
...
mod tests {
    ...
    async fn daemon_state_serialize_deserialize() {
        ...
        assert_eq!(
            error,
            DeserializeError::Deserialize {
                error: Err::Error(NomError::new(input, NomErrorKind::Eof)),
        ...
        assert_eq!(
            error,
            DeserializeError::Deserialize {
                error: Err::Error(NomError::new(vec![42; 10], NomErrorKind::Tag)),
